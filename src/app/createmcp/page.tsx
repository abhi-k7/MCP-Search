...
export default function CreateMCPPage() {
  ...
  async function handleSend() {
    ...
    setCode("");
    try {
      const res = await fetch("/api/ai-mcpbuild", {
        method: "POST",
  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col items-center justify-center min-h-[80vh] px-4 mt-10 pt-20 pb-16">
      <h1 className="text-6xl font-extrabold text-center gradient-text mb-4">MCP Server Template Generator</h1>
      <p className="text-lg text-muted-foreground mb-8">
         Have an idea for an MCP server? Generate a template from a simple description. Only Typescript output currently
          supported.
        </p>
      <form
      ...
      >
        ...
        >
          {isSubmitting ? "Building..." : "Generate Template"}
        </Button>
      </form>
      {error && <div className="text-red-600 mt-4">{error}</div>}
      {code && (
        <div className="w-full max-w-5xl mx-auto mt-8 border border-primary rounded-xl shadow-lg transition-all duration-300 animate-fade-in bg-background">
          <div className="flex items-center gap-2 mb-3 px-6 pt-6">
            <h2 className="text-xl font-semibold gradient-text">Generated MCP Server Template</h2>
          </div>
